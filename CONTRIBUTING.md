...
- 建议启用 Git hooks：`npm run hooks:install`（提交前会自动构建并暂存 `app/dist-extension/`）

## 构建与加载扩展

1. 在 Chrome（或 Chromium 系浏览器）打开 `chrome://extensions/`，开启「开发者模式」
2. 点击「加载已解压的扩展程序」，选择**本仓库根目录**
3. 弹窗入口为 `app/dist-extension/popup.html`（默认随仓库提供）
4. 若你改了 `app/` 下的 React Popup 代码：在项目根执行 `npm run build:extension`，更新 `app/dist-extension/` 后再重载扩展
...
- **Popup 单一来源**：以 `app/src/popup/` 为唯一源码；`app/dist-extension/` 只存放构建产物，不直接手改。
...
- Popup UI 回归：在项目根执行 `npm run test:popup`（需先执行 `npm run build:extension`）
...
- 提交前请在本地完成构建与上述测试
- 推荐提交流程（确保 `app/dist-extension/` 一定被提交）：
