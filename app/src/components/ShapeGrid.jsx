...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = event => {
      ...
      // If listening globally, ignore moves outside canvas bounds and treat as leave.
      if (listenOnWindow) {
    ...
    const moveTarget = listenOnWindow ? window : canvas;
    moveTarget.addEventListener('pointermove', handleMouseMove, { passive: true });
    if (!listenOnWindow) {
    ...
    return () => {
      ...
      moveTarget.removeEventListener('pointermove', handleMouseMove);
      if (!listenOnWindow) {
        canvas.removeEventListener('pointerleave', handleMouseLeave);
      }
    };
  }, [direction, speed, borderColor, hoverFillColor, squareSize, shape, hoverTrailAmount, listenOnWindow]);
